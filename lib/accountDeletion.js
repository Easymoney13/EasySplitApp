...
function deletedMemberId(originalId = '', deletionSeed = '') {
  const suffix = deletionSeed
    ? crypto.createHmac('sha256', deletionSeed).update(String(originalId)).digest('hex').slice(0, 20)
    : crypto.randomBytes(10).toString('hex');
  return `member_deleted_${suffix}`;
}

function anonymizeAccountInRecord(record, uid, sharedReplacements = new Map(), deletionSeed = '') {
  ...
  for (const member of targets) {
    if (member?.id && !replacements.has(String(member.id))) {
      replacements.set(String(member.id), deletedMemberId(member.id, deletionSeed));
    }
  }
  ...
  next.members = (Array.isArray(next.members) ? next.members : []).map((member, index) => {
    ...
    clean.id = replacements.get(String(original.id || '')) || deletedMemberId(uid, deletionSeed);
