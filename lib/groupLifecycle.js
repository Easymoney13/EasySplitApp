...
function resolveActiveGroupMemberId(group, target) {
  if (!target) return null;
  const activeMembers = Array.isArray(group?.members)
    ? group.members.filter((member) => member?.id && member.active !== false)
    : [];
  const byId = activeMembers.find((member) => member.id === target);
  if (byId) return byId.id;
  const normalized = String(target).trim().toLowerCase();
  if (!normalized) return null;
  const byName = activeMembers.filter((member) => String(member.name || '').trim().toLowerCase() === normalized);
...
function isValidPayerId(group, payerId) {
  ...
  return Boolean(resolveActiveGroupMemberId(group, normalized));
}

function hasUnassignedItems(bill, group = null) {
  const items = Array.isArray(bill?.items) ? bill.items : [];
  return items.some((item) => {
    const price = Number(item?.price) || 0;
    const claimants = Array.isArray(item?.claimedBy) ? item.claimedBy.filter(Boolean) : [];
    if (price <= 0) return false;
    if (!group) return claimants.length === 0;
    return !claimants.some((claimant) => Boolean(resolveActiveGroupMemberId(group, claimant)));
  });
}
...
function finalizeGroupBill(group, billId, actor, now = Date.now) {
  ...
  if (hasUnassignedItems(bill, updated)) {
    throw lifecycleError('Assign every priced item before finishing this split');
  }
  const timestamp = now();
  const normalizedPayerId = normalizeGroupPayerId(bill.payerId);
  bill.payerId = normalizedPayerId === 'each'
    ? 'each'
    : resolveActiveGroupMemberId(updated, normalizedPayerId);
...
function beginGroupSettlement(group, actor, now = Date.now) {
  ...
  const invalidPayerBill = financiallyOpenBills.find((bill) => !isValidPayerId(updated, bill.payerId));
  if (invalidPayerBill) {
    throw lifecycleError('Choose a valid payer for every split before starting group settlement');
  }
