...
function normalizeIsraeliPhone(value) {
  ...
  let digits = optionalString(value, 30).replace(/\D/g, '');
...
function validateUserSyncBody(rawBody) {
  ...
  if (typeof body.phone === 'string') {
    const rawPhone = optionalString(body.phone, 30);
    const phone = normalizeIsraeliPhone(rawPhone);
    if (rawPhone && !phone) throw new ValidationError('phone must be a valid Israeli mobile number');
    clean.phone = phone;
  }
  return clean;
}
...
function validateSessionAction(action, rawPayload) {
  ...
  if (['TOGGLE_CLAIM', 'TOGGLE_SETTLED', 'RESOLVE_DELETED_MEMBER'].includes(action)) {
