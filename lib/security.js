...
function sanitizeString(input, maxLength = 100) {
  ...
  return input
    .replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '') // Remove <script>...</script> completely
    .replace(/<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi, '')   // Remove <style>...</style> completely
...
/**
 * Validates current 5-digit session codes, 8-digit group codes, and legacy 4-digit codes.
 */
function isValidRoomCode(code) {
  return /^(?:\d{5}|\d{8}|\d{4})$/.test(code);
}
...
function isValidSessionId(sessionId) {
  return /^(sess_(?:g_)?[a-z0-9_\-]{6,100}|\d{4}|\d{5}|\d{8})$/i.test(String(sessionId || ''));
}
...
function createUserRateLimiter({
  ...
  function middleware(req, res, next) {
    ...
    const key = userId ? `user:${userId}` : `ip:${normalizeIp(rawIp)}`;
