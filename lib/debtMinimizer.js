...
function isFinanciallyOpenBill(bill) {
...
function calculateDebtMinimization(group) {
  const members = Array.isArray(group?.members)
    ? group.members.filter((member) => member?.id && member.active !== false)
    : [];
  ...
  let hasAccountingError = false;

  bills.forEach((bill) => {
    if (!isFinanciallyOpenBill(bill)) return;
    ...
    const rawPayerId = bill?.payerId;
    const hasExplicitPayer = rawPayerId !== null && rawPayerId !== undefined && String(rawPayerId).trim() !== '';
    const resolvedPayerId = eachPaidOwnShare ? null : resolveMemberId(rawPayerId, members);
    const payerId = eachPaidOwnShare ? null : (resolvedPayerId || (!hasExplicitPayer ? memberIds[0] : null));
    if (!eachPaidOwnShare && (!payerId || !records.has(payerId))) {
      hasAccountingError = true;
      return;
    }
    ...
    items.forEach((item, index) => {
      ...
      const rawClaimants = [...new Set(
        (Array.isArray(item?.claimedBy) ? item.claimedBy : []).filter(Boolean)
      )];
      const resolvedClaimants = rawClaimants.map((claimant) => resolveMemberId(claimant, members));
      if (rawClaimants.length === 0 || resolvedClaimants.some((memberId) => !memberId)) {
        unassignedCents += itemCents;
        return;
      }
      const claimantIds = [...new Set(resolvedClaimants)];
  ...
  const rawTransactions = balanceSumCents === 0 && !hasAccountingError
  ...
  return {
    ...
    isBalanced: balanceSumCents === 0 && !hasAccountingError,
  };
}
