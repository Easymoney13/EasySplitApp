...
const PIPELINE_TIMEOUT_MS = 4_200;
...
async function parseReceiptImage(base64Image, mimeType = 'image/jpeg', customApiKey = '', options = {}) {
  ...
  const selectedModels = models.slice(0, FAST_CONSENSUS_MODEL_COUNT);
  ...
  const pendingReads = new Map(selectedModels.map((modelName, index) => {
    const promise = (async () => {
      ...
      return { index, modelName, receipt };
    })();
    return [promise, index];
  }));
  const successfulReads = [];
  while (pendingReads.size > 0) {
    const completed = await Promise.race([...pendingReads.keys()].map((promise) => (
      promise.then((read) => ({ promise, read }))
    )));
    pendingReads.delete(completed.promise);
    if (completed.read.receipt) successfulReads.push(completed.read);
    // Two identical independent reads are already an irreversible 2-of-3
    // majority. Return immediately instead of waiting for the slowest model.
  ...
  return {
    ...
    ocr: receiptOcrEvidence(selectedReceipt, {
      ...
      modelAttempts: selectedModels.length,
