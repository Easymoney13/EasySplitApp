...
function normalizeReceipt(parsed, sourceText) {
  ...
  const normalized = {
    ...
    restaurant: (() => {
      const rawRes = (parsed.restaurant && typeof parsed.restaurant === 'object') ? parsed.restaurant : {};
      const businessId = typeof (rawRes.businessId || rawRes.taxId || rawRes.companyId || parsed.businessId || parsed.taxId) === 'string'
        ? String(rawRes.businessId || rawRes.taxId || rawRes.companyId || parsed.businessId || parsed.taxId).trim()
        : '';
      const address = typeof (rawRes.address || parsed.address || rawRes.location) === 'string'
        ? normalizeOcrName(rawRes.address || parsed.address || rawRes.location)
        : '';
      const phone = typeof (rawRes.phone || parsed.phone || rawRes.telephone) === 'string'
        ? String(rawRes.phone || parsed.phone || rawRes.telephone).trim()
        : '';
      const printedName = typeof (rawRes.printedName || rawRes.name || parsed.storeName) === 'string'
        ? normalizeOcrName(rawRes.printedName || rawRes.name || parsed.storeName)
        : '';
      return {
        printedName,
        businessId,
...
async function fetchJsonWithTimeout(url, options, timeoutMs = REQUEST_TIMEOUT_MS, externalSignal = null) {
  ...
  try {
    ...
    if (!response.ok) {
      // The caller only needs the status; do not leave an unread error stream
      // running while the pipeline moves on to another provider.
      controller.abort();
      return { ok: false, status: response.status, data: null };
    }
    // fetch resolves at the headers. Keep both the deadline and quorum abort
    // connected until JSON consumption finishes, including a stalled body.
...
- Preserve restaurant identity separately in restaurant.printedName, restaurant.businessId (look for ח.פ / ע.מ / עוסק מורשה / tax ID), restaurant.address (street, city, or branch name), and restaurant.phone (business phone/tel) whenever visibly printed. Never guess an unreadable identity field.
...
function normalizeRestaurantValue(field, value) {
  const text = String(value || '').normalize('NFKC').trim().toLowerCase();
  if (field === 'businessId' || field === 'phone') return text.replace(/\D/g, '');
  return text.replace(/[^\p{L}\p{N}]+/gu, ' ').replace(/\s+/g, ' ').trim();
}

function haveSameRestaurantIdentity(first, second) {
  return ['printedName', 'businessId', 'address', 'phone'].every((field) => (
    normalizeRestaurantValue(field, first?.restaurant?.[field])
      === normalizeRestaurantValue(field, second?.restaurant?.[field])
  ));
}

function haveSameReceiptValues(first, second) {
  if (!haveSameLineIdentities(first, second)) return false;
  if (!haveSameRestaurantIdentity(first, second)) return false;
...
function majorityRestaurantValue(field, receipts) {
  const candidates = receipts
    .map((receipt) => ({
      raw: String(receipt?.restaurant?.[field] || '').trim(),
      normalized: normalizeRestaurantValue(field, receipt?.restaurant?.[field]),
    }))
    .filter((candidate) => candidate.normalized);
  const counts = new Map();
  for (const candidate of candidates) {
    const current = counts.get(candidate.normalized) || { count: 0, raw: candidate.raw };
    counts.set(candidate.normalized, { count: current.count + 1, raw: current.raw || candidate.raw });
  }
  const winner = [...counts.values()].find((candidate) => candidate.count >= 2);
  return winner?.raw || '';
}
...
function buildValueConsensus(primary, verification, tiebreaker) {
  ...
  let verifiedRestaurantFields = 0;
  const fieldVerification = {};
  for (const field of ['printedName', 'businessId', 'address', 'phone']) {
    const value = majorityRestaurantValue(field, [primary, verification, tiebreaker]);
    if (!value) {
      // Never let a primary-only value inherit trust from consensus on a
      // different field (for example, agreeing names with three conflicting
      // business identifiers).
      delete restaurant[field];
      if (field === 'businessId') delete restaurant.taxId;
      fieldVerification[field] = 'unresolved';
      continue;
    }
    verifiedRestaurantFields += 1;
    fieldVerification[field] = 'verified';
    if (normalizeRestaurantValue(field, restaurant[field]) !== normalizeRestaurantValue(field, value)) changedValues += 1;
    restaurant[field] = value;
  }
  receipt.restaurant = {
    ...restaurant,
    consensusStatus: verifiedRestaurantFields > 0 ? 'verified' : 'unverified',
    fieldVerification,
  };
  ...
  return { receipt, resolvedItemPrices, unresolvedItemPrices, changedValues, verifiedRestaurantFields };
}
...
async function parseReceiptImage(base64Image, mimeType = 'image/jpeg', customApiKey = '', options = {}) {
  ...
  if (exactAgreement) {
    ...
    if (correctedPrimaryValues) {
      ...
      consensusEvidence = {
        ...
        verifiedRestaurantFields: consensus.verifiedRestaurantFields,
      };
      ...
      selectedRead = {
        ...selectedRead,
        receipt: {
          ...selectedRead.receipt,
          restaurant: {
            ...(selectedRead.receipt.restaurant || {}),
  ...
  const evidenceReads = [selectedRead, ...reads.filter((read) => read.index !== selectedRead.index)];
  return {
    ...selectedReceipt,
