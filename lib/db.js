...
const { normalizeIsraeliPhone, sanitizeUserSettings } = require('./validation');
const { anonymizeAccountInRecord } = require('./accountDeletion');
const { hasFinishedShare } = require('./sessionActions');
const { resolveRestaurantAliases } = require('./restaurantResolution');
const {
  assessVisitData,
  createIdentityHmac,
...
} = require('./restaurantDataFoundation');
const {
  GROUP_STATUS,
  BILL_STATUS,
  getGroupStatus,
...
// A minimal security tombstone survives the temporary cleanup job. Without it,
// a request admitted before deletion can recreate identity after the scan ends.
// Its key is a namespaced hash; its value contains only the deletion timestamp.
function accountFenceId(uid) {
  return crypto.createHash('sha256').update(`easysplit-account-deletion-v1:${uid}`).digest('hex');
}

function accountWriteError() {
  return Object.assign(new Error('This request includes an account whose deletion has started. Refresh and try again.'), {
    statusCode: 409, errorCode: 'ACCOUNT_DELETED',
  });
}

function recordAccountIds(...records) {
  const ids = new Set();
  for (const record of records.filter(Boolean)) {
    for (const member of Array.isArray(record.members) ? record.members : []) {
      const uid = memberAccountId(member);
      if (uid) ids.add(uid);
    }
    for (const uid of Array.isArray(record.memberIds) ? record.memberIds : []) if (uid) ids.add(uid);
    if (record.userId) ids.add(record.userId);
    if (record.submittedByUserId) ids.add(record.submittedByUserId);
  }
  return [...ids];
}

function assertLocalAccountWritesAllowed(data, uids) {
  for (const uid of new Set(uids.filter(Boolean))) {
    if (data.deletedAccountFences?.[accountFenceId(uid)]) throw accountWriteError();
  }
}

async function assertFirestoreAccountWritesAllowed(firestore, transaction, uids) {
  // Do not cache across callbacks: Firestore may reuse its transaction object
  // on retry, and a deletion fence can have appeared since the previous read.
  for (const uid of new Set(uids.filter(Boolean))) {
    const key = accountFenceId(uid);
    const fence = await transaction.get(firestore.collection('_deleted_account_fences').doc(key));
    if (fence.exists) throw accountWriteError();
  }
}

async function updateFirestoreHistoryProfiles(firestore, historyRecord) {
  for (const member of Array.isArray(historyRecord.members) ? historyRecord.members : []) {
    const userRef = firestore.collection('users').doc(member.id);
    await firestore.runTransaction(async (transaction) => {
      const snapshot = await transaction.get(userRef);
      if (!snapshot.exists) return;
      await assertFirestoreAccountWritesAllowed(firestore, transaction, [member.id, ...recordAccountIds(historyRecord)]);
      const bills = Array.isArray(snapshot.data().bills) ? [...snapshot.data().bills] : [];
      const index = bills.findIndex((bill) => bill.id === historyRecord.id);
      if (index < 0) bills.unshift(historyRecord);
      else bills[index] = { ...bills[index], ...historyRecord };
      transaction.set(userRef, sanitizeFirestoreData({ bills, updatedAt: Date.now() }), { merge: true });
    });
  }
}
...
function synchronizeSessionMembersWithGroup(session, groupMembers) {
  ...
  const participantSetFrozen = session?.status === 'settled'
    || [...existingMembers.values()].some((member) => member.active !== false && hasFinishedShare(member));
  if (participantSetFrozen) {
    ...
    return [...existingMembers.values()].map((existingMember) => ({
      ...
      ...(existingMember.deletionResolution ? { deletionResolution: existingMember.deletionResolution } : {}),
    }));
  }
  return (Array.isArray(groupMembers) ? groupMembers : []).map((groupMember) => ({
    ...
    ...(existingMembers.get(groupMember.id)?.deletionResolution
      ? { deletionResolution: existingMembers.get(groupMember.id).deletionResolution } : {}),
  }));
}
...
function phoneIdentityHmac(normalizedPhone) {
  const secret = process.env.EASYSPLIT_IDENTITY_HMAC_SECRET
    || (isTesting ? 'easysplit-test-only-identity-secret' : '');
  return createIdentityHmac(secret, normalizedPhone);
}

let lastRestaurantDataProtectionWarningAt = 0;

function warnRestaurantDataProtectionDegraded(visitId, issues) {
  if (isTesting || !issues?.includes('missing_phone_hmac')) return;
  const now = Date.now();
  if (now - lastRestaurantDataProtectionWarningAt < 5 * 60 * 1000) return;
  lastRestaurantDataProtectionWarningAt = now;
  console.warn(JSON.stringify({
    event: 'restaurant_data_protection_degraded',
    visitId,
    issues,
    action: 'configure_EASYSPLIT_IDENTITY_HMAC_SECRET',
  }));
}

function enrichUserPhoneIdentity(user) {
  const normalizedPhone = normalizeIsraeliPhone(user?.phone || '');
  const phoneHmac = phoneIdentityHmac(normalizedPhone);
  ...
  return sanitizeFirestoreData({
    ...
    phoneAssurance,
    phoneDataQualityStatus: normalizedPhone
      ? (phoneHmac ? 'complete' : 'blocked_missing_hmac_secret')
      : (user?.phone ? 'invalid' : 'missing'),
...
function buildRestaurantVisit(session, member, observedAt = Date.now()) {
  ...
  const normalizedPhone = normalizeIsraeliPhone(
    member.phone || (member.isHost ? session.hostPhone || '' : ''),
  );
  const phoneHmac = phoneIdentityHmac(normalizedPhone);
  const occurredAt = timestampMillis(member.joinedAt)
    || timestampMillis(session.createdAt)
    || timestampMillis(observedAt)
    || Date.now();
  const visitDate = normalizeVisitDate(session.date)
  ...
  const visit = sanitizeFirestoreData({
    ...
    joinedAt: timestampMillis(member.joinedAt) || timestampMillis(observedAt) || occurredAt,
  ...
  warnRestaurantDataProtectionDegraded(visitId, quality.dataQualityIssues);
  return visit;
}
...
async function resumeFirestoreGroupDeletion(firestore, groupId) {
  ...
  for (const userId of groupUserIds) {
    // A delayed group cleanup must never recreate a deleted profile.
    const userRef = firestore.collection('users').doc(userId);
    await firestore.runTransaction(async (transaction) => {
      const user = await transaction.get(userRef);
      const fence = await transaction.get(firestore.collection('_deleted_account_fences').doc(accountFenceId(userId)));
      if (!user.exists || fence.exists) return;
      transaction.set(userRef, { groups: fieldValue.arrayRemove(groupId), updatedAt: Date.now() }, { merge: true });
    });
  }
...
function mergeRestaurantVisit(existing, incoming) {
  ...
  return sanitizeFirestoreData({
    ...
    // Never retain a weak legacy SHA phone hash or a stale HMAC after a phone
    // correction/configuration failure. The next valid write can restore it.
    phoneHash: undefined,
    phoneHmac: incoming.phoneHmac,
    ...
    occurredAt: prior.occurredAt || incoming.occurredAt,
    visitDate: prior.visitDate || incoming.visitDate,
...
function applyLocalRestaurantVisits(data, session, members, observedAt = Date.now(), { upsertRestaurant = true } = {}) {
  assertLocalAccountWritesAllowed(data, recordAccountIds({ members }));
...
async function prepareFirestoreRestaurantVisits(
  ...
  if (!session?.restaurant?.id) return [];
  await assertFirestoreAccountWritesAllowed(firestore, transaction, recordAccountIds({ members }));
  const visits = (members || []).map((member) => buildRestaurantVisit(session, member, observedAt)).filter(Boolean);
  if (!visits.length) return [];
...
const db = {
  ...
  saveUser(user, trustedUid = '') {
    ...
    if (!uid) return null;
    const normalizedUser = user?.phone !== undefined
      ? enrichUserPhoneIdentity({ ...user, id: uid })
      : sanitizeFirestoreData({ ...user, id: uid });
    if (isTesting) {
      const data = readDb();
      assertLocalAccountWritesAllowed(data, [uid, ...recordAccountIds(...(Array.isArray(normalizedUser.bills) ? normalizedUser.bills : []))]);
      data.users[uid] = { ...data.users[uid], ...normalizedUser, updatedAt: Date.now() };
      writeDb(data);
    ...
    return (async () => {
      ...
      const data = {
        ...normalizedUser,
        ...(normalizedUser.username !== undefined ? { username_lowercase } : {}),
        updatedAt: Date.now()
      };
      const firestore = getFirestore();
      await firestore.runTransaction(async (transaction) => {
        await assertFirestoreAccountWritesAllowed(firestore, transaction, [uid, ...recordAccountIds(...(Array.isArray(normalizedUser.bills) ? normalizedUser.bills : []))]);
        transaction.set(firestore.collection('users').doc(uid), data, { merge: true });
      });
      return data;
    })();
  },
  ...
  addGroupToUser(uid, groupId) {
    ...
    if (isTesting) {
      const data = readDb();
      assertLocalAccountWritesAllowed(data, [uid]);
    ...
    return (async () => {
      ...
      return firestore.runTransaction(async (transaction) => {
        await assertFirestoreAccountWritesAllowed(firestore, transaction, [uid]);
  ...
  findOrCreateUser(uid, username, phone, settings = {}) {
    ...
    return (async () => {
      ...
      return firestore.runTransaction(async (transaction) => {
        ...
        const defaults = {
          language: 'en', currency: 'NIS', theme: 'light',
          customGeminiKey: '', ocrEngine: 'tesseract',
        };
        const profile = enrichUserPhoneIdentity({
          id: uid,
          username: username || current?.username || 'User',
          phone: phone || current?.phone || '',
          phoneVerificationStatus: current?.phoneVerificationStatus,
          phoneVerifiedAt: current?.phoneVerifiedAt,
          settings: { ...(current?.settings || defaults), ...safeSettings },
        });
        const patch = {
          ...profile,
          username_lowercase: profile.username.toString().trim().toLowerCase(),
          updatedAt: Date.now(),
          ...(!current ? { avatarColor: '#7C3AED', bills: [], groups: [], createdAt: Date.now() } : {}),
        };
        // Only profile fields are written. Group and history indices have their
        // own transactional writers and must never come from a stale profile.
  ...
  updateUserSettings(uid, username, phone, newSettings) {
    ...
    return this.findOrCreateUser(uid, username, phone, safeSettings);
  },

  addUserBill(uid, username, phone, billRecord) {
    if (isTesting) {
      assertLocalAccountWritesAllowed(readDb(), [uid, ...recordAccountIds(billRecord)]);
      const user = this.findOrCreateUser(uid, username, phone);
      if (user) {
    ...
    return (async () => {
      ...
      return firestore.runTransaction(async (transaction) => {
        await assertFirestoreAccountWritesAllowed(firestore, transaction, [user.id, ...recordAccountIds(billRecord)]);
        const snapshot = await transaction.get(userRef);
        if (!snapshot.exists) return null;
        const current = snapshot.data();
        const bills = Array.isArray(current.bills) ? [...current.bills] : [];
        const index = bills.findIndex((bill) => bill.id === billRecord.id);
        if (index >= 0) bills[index] = { ...bills[index], ...billRecord };
        else bills.unshift(billRecord);
        transaction.set(userRef, sanitizeFirestoreData({ bills, updatedAt: Date.now() }), { merge: true });
  ...
  isAccountDeletionComplete(uid) {
    if (!uid) return false;
    if (isTesting) {
      const data = readDb();
      return Boolean(data.deletedAccountFences?.[accountFenceId(uid)] && !data.users?.[uid]);
    }
    const firestore = getFirestore();
    return firestore.runTransaction(async (transaction) => {
      const [fence, job, user] = await Promise.all([
        transaction.get(firestore.collection('_deleted_account_fences').doc(accountFenceId(uid))),
        transaction.get(firestore.collection('_account_deletions').doc(uid)),
        transaction.get(firestore.collection('users').doc(uid)),
      ]);
      return fence.exists && !job.exists && !user.exists;
    });
  },

  deleteUserAccountData(uid) {
    if (!uid) return { deleted: false, anonymizedRecords: 0, deletedVisits: 0 };
    if (isTesting) {
      const data = readDb();
      const user = data.users?.[uid] || null;
      if (!data.deletedAccountFences) data.deletedAccountFences = {};
      data.deletedAccountFences[accountFenceId(uid)] ||= { deletedAt: Date.now() };
      const phoneHmac = user?.phoneHmac || (user?.phone ? phoneIdentityHmac(normalizeIsraeliPhone(user.phone)) : '');
      const accountMemberReplacements = new Map();
      let anonymizedRecords = 0;
      for (const collectionName of ['groups', 'sessions']) {
        for (const [id, record] of Object.entries(data[collectionName] || {})) {
          const result = anonymizeAccountInRecord(record, uid, accountMemberReplacements);
          if (!result.changed) continue;
          data[collectionName][id] = result.record;
          anonymizedRecords += 1;
        }
      }
      data.history = (data.history || []).map((record) => {
        const result = anonymizeAccountInRecord(record, uid, accountMemberReplacements);
        if (result.changed) anonymizedRecords += 1;
        return result.record;
      });
      for (const [otherUid, pointers] of Object.entries(data.historyPointers || {})) {
        if (otherUid === uid || !Array.isArray(pointers)) continue;
        data.historyPointers[otherUid] = pointers.map((record) => {
          const result = anonymizeAccountInRecord(record, uid, accountMemberReplacements);
          if (result.changed) anonymizedRecords += 1;
          return result.record;
        });
      ...
      // Older versions embedded shared history snapshots in each participant's
      // profile. Keep the other account and its financial records, but scrub the
      // deleted identity from those copies just like canonical shared history.
      for (const [otherUid, otherUser] of Object.entries(data.users || {})) {
        if (otherUid === uid || !Array.isArray(otherUser.bills)) continue;
        otherUser.bills = otherUser.bills.map((bill) => {
          const result = anonymizeAccountInRecord(bill, uid, accountMemberReplacements);
          if (result.changed) anonymizedRecords += 1;
          return result.record;
        });
      }
      ...
      for (const [id, visit] of Object.entries(data.restaurantVisits || {})) {
        const aliases = Array.isArray(visit?.identityAliases) ? visit.identityAliases : [];
        if (visit?.userId === uid || aliases.includes(`user:${uid}`) || (phoneHmac && visit?.phoneHmac === phoneHmac)) {
          deletedVisitIds.add(id);
          delete data.restaurantVisits[id];
        }
      }
      for (const observation of Object.values(data.restaurantObservations || {})) {
        if (!observation || typeof observation !== 'object') continue;
        if (observation.submittedByUserId === uid) delete observation.submittedByUserId;
        if (deletedVisitIds.has(observation.submittedByVisitId)) delete observation.submittedByVisitId;
        if (Array.isArray(observation.observedByVisitIds)) {
          observation.observedByVisitIds = observation.observedByVisitIds.filter((id) => !deletedVisitIds.has(id));
        }
      }

      if (data.users) delete data.users[uid];
      if (data.historyPointers) delete data.historyPointers[uid];
      writeDb(data);
      return {
        deleted: Boolean(user),
        anonymizedRecords,
        deletedVisits: deletedVisitIds.size,
      };
    }
    ...
    return (async () => {
      const firestore = getFirestore();
      const userRef = firestore.collection('users').doc(uid);
      // This private, temporary job exists only until cleanup succeeds. Its
      // random seed makes linked anonymous IDs stable across transaction retries
      // and a process restart after partially completed cleanup.
      const jobRef = firestore.collection('_account_deletions').doc(uid);
      const fenceRef = firestore.collection('_deleted_account_fences').doc(accountFenceId(uid));
      const { userSnapshot, job } = await firestore.runTransaction(async (transaction) => {
        const [snapshot, jobSnapshot, fenceSnapshot] = await Promise.all([
          transaction.get(userRef), transaction.get(jobRef), transaction.get(fenceRef),
        ]);
        const user = snapshot.exists ? snapshot.data() : null;
        const existingJob = jobSnapshot.exists ? jobSnapshot.data() : null;
        const job = existingJob || {
          seed: crypto.randomBytes(32).toString('hex'),
          phoneHmac: user?.phoneHmac || (user?.phone ? phoneIdentityHmac(normalizeIsraeliPhone(user.phone)) : ''),
          startedAt: Date.now(),
        };
        if (!fenceSnapshot.exists) transaction.set(fenceRef, { deletedAt: job.startedAt || Date.now() });
        if (!existingJob) transaction.set(jobRef, job);
        return { userSnapshot: snapshot, job };
      });
      const accountMemberReplacements = new Map();
      let anonymizedRecords = 0;

      // The collection scan discovers candidates only. Read and transform the
      // CURRENT document inside its transaction, never write a scanned snapshot.
      for (const collectionName of ['groups', 'sessions', 'history']) {
        // Legacy history subcollections can hold complete shared snapshots,
        // including below a missing profile. Pointer-only rows need no change.
        const collection = collectionName === 'history'
          ? firestore.collectionGroup('history')
          : firestore.collection(collectionName);
        const snapshot = await collection.get();
        for (const doc of snapshot.docs) {
          if (!anonymizeAccountInRecord(doc.data(), uid, accountMemberReplacements, job.seed).changed) continue;
          const changed = await firestore.runTransaction(async (transaction) => {
            const current = await transaction.get(doc.ref);
            if (!current.exists) return false;
            const result = anonymizeAccountInRecord(current.data(), uid, accountMemberReplacements, job.seed);
            if (!result.changed) return false;
            transaction.set(doc.ref, sanitizeFirestoreData(result.record));
            return true;
          });
          if (changed) anonymizedRecords += 1;
        }
      }

      const profiles = await firestore.collection('users').get();
      for (const doc of profiles.docs) {
        if (doc.id === uid || !Array.isArray(doc.data()?.bills)) continue;
        if (!doc.data().bills.some((bill) => anonymizeAccountInRecord(bill, uid, accountMemberReplacements, job.seed).changed)) continue;
        const changed = await firestore.runTransaction(async (transaction) => {
          const current = await transaction.get(doc.ref);
          if (!current.exists || !Array.isArray(current.data()?.bills)) return 0;
          let changedBills = 0;
          const bills = current.data().bills.map((bill) => {
            const result = anonymizeAccountInRecord(bill, uid, accountMemberReplacements, job.seed);
            if (result.changed) changedBills += 1;
            return result.record;
          });
          if (changedBills) transaction.set(doc.ref, sanitizeFirestoreData({ bills }), { merge: true });
          return changedBills;
        });
        anonymizedRecords += changed;
      }

      const deletedVisitIds = new Set();
      const visitRefs = new Map();
      const visitSnapshots = [
        await firestore.collection('restaurant_visits').where('userId', '==', uid).get(),
        await firestore.collection('restaurant_visits').where('identityAliases', 'array-contains', `user:${uid}`).get(),
      ];
      if (job.phoneHmac) {
        visitSnapshots.push(await firestore.collection('restaurant_visits').where('phoneHmac', '==', job.phoneHmac).get());
      }
      for (const snapshot of visitSnapshots) {
        for (const doc of snapshot.docs) {
          deletedVisitIds.add(doc.id);
          visitRefs.set(doc.id, doc.ref);
        }
      }

      // Remove references BEFORE deleting visits. If interrupted, the next run
      // can still discover every visit whose references have not been removed.
      const observations = await firestore.collection('restaurant_observations').get();
      for (const doc of observations.docs) {
        await firestore.runTransaction(async (transaction) => {
          const current = await transaction.get(doc.ref);
          if (!current.exists) return;
          const observation = { ...current.data() };
          let changed = false;
          if (observation.submittedByUserId === uid) {
            delete observation.submittedByUserId;
            changed = true;
          }
          if (deletedVisitIds.has(observation.submittedByVisitId)) {
            delete observation.submittedByVisitId;
            changed = true;
          }
          if (Array.isArray(observation.observedByVisitIds)) {
            const filtered = observation.observedByVisitIds.filter((id) => !deletedVisitIds.has(id));
            if (filtered.length !== observation.observedByVisitIds.length) {
              observation.observedByVisitIds = filtered;
              changed = true;
            }
          }
          if (changed) transaction.set(doc.ref, sanitizeFirestoreData(observation));
        });
      }

      const writer = createBoundedFirestoreBatchWriter(firestore);
      for (const ref of visitRefs.values()) await writer.enqueue((batch) => batch.delete(ref));
      const historyPointers = await userRef.collection('history').get();
      for (const doc of historyPointers.docs) await writer.enqueue((batch) => batch.delete(doc.ref));
      await writer.flush();
      // Keep the job and profile until every preceding cleanup write succeeded.
      // Final removal is atomic so retries never lose the anonymous ID seed.
      await firestore.runTransaction(async (transaction) => {
        const [currentJob, currentUser] = await Promise.all([transaction.get(jobRef), transaction.get(userRef)]);
        if (!currentJob.exists && !currentUser.exists) return;
        if (!currentJob.exists || currentJob.data()?.seed !== job.seed) {
          throw Object.assign(new Error('Account cleanup changed; retry deletion'), { statusCode: 409 });
        }
        transaction.delete(userRef);
        transaction.delete(jobRef);
      });
      return {
        deleted: userSnapshot.exists,
        anonymizedRecords,
        deletedVisits: deletedVisitIds.size,
      };
    })();
  ...
  saveSession(session) {
    ...
    if (isTesting) {
      const data = readDb();
      assertLocalAccountWritesAllowed(data, recordAccountIds(session));
    ...
    return (async () => {
      ...
      const firestore = getFirestore();
      await firestore.runTransaction(async (transaction) => {
        await assertFirestoreAccountWritesAllowed(firestore, transaction, recordAccountIds(session));
        transaction.set(firestore.collection('sessions').doc(id), data, { merge: true });
      });
      return data;
    })();
  },
  ...
  createSessionIfAbsent(session, { restaurantVisitMembers = [], restaurantProofId = '' } = {}) {
    ...
    if (isTesting) {
      const data = readDb();
      assertLocalAccountWritesAllowed(data, recordAccountIds(session, { members: restaurantVisitMembers }));
    ...
    return (async () => {
      ...
      return firestore.runTransaction(async (transaction) => {
        await assertFirestoreAccountWritesAllowed(firestore, transaction, recordAccountIds(session, { members: restaurantVisitMembers }));
  ...
  transactSessionAndLinkedGroup(sessionId, transform, { manualAdmissionCode = '' } = {}) {
    if (isTesting) {
      ...
      assertLocalAccountWritesAllowed(data, recordAccountIds(result.session, result.group, result.history));
    ...
    return (async () => {
      ...
      return firestore.runTransaction(async (transaction) => {
        ...
        if (!result?.session) return null;
        await assertFirestoreAccountWritesAllowed(firestore, transaction, recordAccountIds(result.session, result.group, result.history));
        const updatedAt = Date.now();
        assertSessionStorageBudget(result.session);
        if (result.group) assertGroupStorageBudget(result.group);
  ...
  transactGroupMembership(groupId, transform) {
    if (isTesting) {
      ...
      assertLocalAccountWritesAllowed(data, recordAccountIds(updatedGroup));
    ...
    return (async () => {
      ...
      return firestore.runTransaction(async (transaction) => {
        ...
        if (!updatedGroup) return null;
        const sessionsToWrite = sessionSnapshots.filter((snapshot) => snapshot.exists).map((snapshot) => {
          const session = snapshot.data();
          return { ...session, members: synchronizeSessionMembersWithGroup(session, updatedGroup.members) };
        });
        await assertFirestoreAccountWritesAllowed(firestore, transaction, recordAccountIds(updatedGroup, ...sessionsToWrite));
        assertGroupStorageBudget(updatedGroup);
        const updatedAt = Date.now();
  ...
  transactGroupAndLinkedSession(groupId, sessionIdResolver, transform) {
    if (isTesting) {
      ...
      assertLocalAccountWritesAllowed(data, recordAccountIds(result.group, result.session));
    ...
    return (async () => {
      ...
      return firestore.runTransaction(async (transaction) => {
        ...
        if (!result?.group) return null;
        await assertFirestoreAccountWritesAllowed(firestore, transaction, recordAccountIds(result.group, result.session));
      ...
      return firestore.runTransaction(async (transaction) => {
        ...
        const bucket = !prior || now >= priorExpiresAt
          ? {
              count: 0,
              startedAt: now,
              expiresAt: Timestamp.fromMillis(retryAt),
            }
          : prior;
  ...
  queryRestaurantAudience(restaurantId, from, to) {
    const safeRestaurantId = typeof restaurantId === 'string' && /^rest_[a-f0-9]{24}$/.test(restaurantId)
      ? restaurantId
      : '';
    if (!safeRestaurantId) {
      const error = new Error('A valid restaurant ID is required');
    ...
    if (isTesting) {
      const data = readDb();
      const restaurant = data.restaurants?.[safeRestaurantId] || null;
      if (!restaurant) return null;
      const aliases = resolveRestaurantAliases(Object.values(data.restaurants || {}), safeRestaurantId);
      const sourceDeletions = Object.values(data.restaurantVisitSourceDeletions || {});
      return {
        restaurant: {
          id: restaurant.id,
          ...aliases,
          printedName: restaurant.printedName || '',
          identityBasis: restaurant.identityBasis || 'unresolved',
          confidence: Number(restaurant.confidence || 0),
          venueResolutionStatus: isResolvedRestaurant(restaurant) ? 'resolved' : 'unresolved',
        },
        ...summarizeRestaurantAudience(Object.values(data.restaurantVisits || {}), sourceDeletions, {
          restaurantId: safeRestaurantId,
          restaurantIds: aliases.restaurantIds,
          restaurant,
          from: fromMs,
          to: toMs,
        }),
    ...
    return (async () => {
      const firestore = getFirestore();
      const restaurantSnapshot = await firestore.collection('restaurants').doc(safeRestaurantId).get();
      if (!restaurantSnapshot.exists) return null;
      const restaurant = { id: restaurantSnapshot.id, ...restaurantSnapshot.data() };
      // Never infer aliases from a partial catalog: omitted conflicts could make
      // an otherwise ambiguous match look safe. This administrative query is bounded.
      const catalog = await firestore.collection('restaurants').limit(10_001).get();
      const aliases = catalog.size > 10_000
        ? { canonicalId: safeRestaurantId, restaurantIds: [safeRestaurantId], resolution: 'catalog_limit' }
        : resolveRestaurantAliases(catalog.docs.map((doc) => ({ ...doc.data(), id: doc.id })), safeRestaurantId)
          || { canonicalId: safeRestaurantId, restaurantIds: [safeRestaurantId], resolution: 'review_required' };
      const visits = [];
      let truncated = false;
      for (const aliasId of aliases.restaurantIds) {
        const querySnapshot = await firestore.collection('restaurant_visits')
          .where('restaurantId', '==', aliasId)
          .where('occurredAt', '>=', fromMs)
          .where('occurredAt', '<', toMs)
          .orderBy('occurredAt', 'asc')
          .limit(10_001 - visits.length)
          .get();
        visits.push(...querySnapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
        if (visits.length > 10_000) {
          visits.length = 10_000;
          truncated = true;
          break;
        }
      }
      const sessionIds = [...new Set(visits.map((visit) => visit.sessionId).filter(Boolean))];
      const deletedSources = new Set();
      for (let index = 0; index < sessionIds.length; index += 250) {
        const references = sessionIds.slice(index, index + 250).map((sessionId) => (
          firestore.collection('restaurant_visit_source_deletions').doc(sessionId)
        ));
        if (!references.length) continue;
        const snapshots = await firestore.getAll(...references);
        for (const snapshot of snapshots) {
          if (snapshot.exists) deletedSources.add(snapshot.id);
        }
      }
      return {
        restaurant: {
        ...
        ...summarizeRestaurantAudience(visits, deletedSources, {
          restaurantId: safeRestaurantId,
          restaurantIds: aliases.restaurantIds,
          restaurant,
          from: fromMs,
  ...
  addToHistory(historyRecord) {
    if (isTesting) {
      const data = readDb();
      assertLocalAccountWritesAllowed(data, recordAccountIds(historyRecord));
    ...
    return (async () => {
      ...
      await db.runTransaction(async (transaction) => {
        await assertFirestoreAccountWritesAllowed(db, transaction, recordAccountIds(historyRecord));
        transaction.set(db.collection('history').doc(id), cleanRecord);
      });
      await updateFirestoreHistoryProfiles(db, cleanRecord);
  ...
  saveSessionAndHistory(session, historyRecord) {
    if (isTesting) {
      const data = readDb();
      assertLocalAccountWritesAllowed(data, recordAccountIds(session, historyRecord));
      data.sessions[session.id] = { ...session, updatedAt: Date.now() };
    ...
    return (async () => {
      ...
      await db.runTransaction(async (transaction) => {
        await assertFirestoreAccountWritesAllowed(db, transaction, recordAccountIds(session, historyRecord));
        transaction.set(db.collection('sessions').doc(session.id), cleanSession, { merge: true });
        transaction.set(db.collection('history').doc(historyRecord.id), cleanHistory);
      });
      await updateFirestoreHistoryProfiles(db, cleanHistory);
      return session;
    })();
  },
  ...
  deleteHistory(id) {
    ...
    return (async () => {
      ...
      for (const doc of usersSnap.docs) {
        await db.runTransaction(async (transaction) => {
          const current = await transaction.get(doc.ref);
          if (!current.exists || !Array.isArray(current.data()?.bills)) return;
          const bills = current.data().bills.filter((bill) => bill.id !== id);
          if (bills.length !== current.data().bills.length) {
            transaction.set(doc.ref, sanitizeFirestoreData({ bills }), { merge: true });
          }
        });
      }
      return this.getHistory();
  ...
  saveGroup(group) {
    ...
    if (isTesting) {
      const data = readDb();
      assertLocalAccountWritesAllowed(data, recordAccountIds(group));
      const id = group.id;
      if (!id) return null;
    ...
    return (async () => {
      ...
      await db.runTransaction(async (transaction) => {
        await assertFirestoreAccountWritesAllowed(db, transaction, recordAccountIds(group));
      ...
      if (Array.isArray(group.members)) {
        for (const member of group.members) {
          ...
          const targetUid = memberAccountId(member);
          if (targetUid) await this.addGroupToUser(targetUid, group.id);
        }
      }
      return data;
    })();
  ...
  saveGroupAndSession(group, session) {
    ...
    if (isTesting) {
      const data = readDb();
      assertLocalAccountWritesAllowed(data, recordAccountIds(group, session));
      const updatedAt = Date.now();
    ...
    return (async () => {
      ...
      await db.runTransaction(async (transaction) => {
        await assertFirestoreAccountWritesAllowed(db, transaction, recordAccountIds(group, session));
        transaction.set(db.collection('groups').doc(group.id), cleanGroup, { merge: true });
        transaction.set(db.collection('sessions').doc(session.id), cleanSession, { merge: true });
      });
  ...
  saveGroupBillAndSession(groupId, bill, session, actorId, expectedRevision = null, {
    ...
    if (isTesting) {
      ...
      if (currentSession?.members?.some((member) => member.active !== false && hasFinishedShare(member))) {
        const error = new Error('Payment allocations are locked while a participant share is finished');
        error.statusCode = 409;
        throw error;
      }
      ...
      assertLocalAccountWritesAllowed(data, recordAccountIds(currentGroup, merged.session));
      const updatedAt = Date.now();
    ...
    return (async () => {
      ...
      const sessionRef = firestore.collection('sessions').doc(session.id);
      return firestore.runTransaction(async (transaction) => {
        // Include the incoming member identities: a stale pre-deletion request
        // can also carry hostPhone/creator fields not present in merged members.
        await assertFirestoreAccountWritesAllowed(firestore, transaction, recordAccountIds(session, { members: restaurantVisitMembers }));
        const groupSnapshot = await transaction.get(groupRef);
        if (!groupSnapshot.exists) return null;
        const sessionSnapshot = await transaction.get(sessionRef);
        ...
        await assertFirestoreAccountWritesAllowed(firestore, transaction, recordAccountIds(cleanGroup, cleanSession));
        const restaurantWrites = await prepareFirestoreRestaurantVisits(
  ...
  leaveGroup(groupId, memberId) {
    if (isTesting) {
      ...
      const settlementStartedInLinkedSession = (group.bills || []).some((bill) => {
        ...
        return linkedSession?.status !== 'settled'
          && linkedSession?.members?.some((candidate) => candidate.active !== false && hasFinishedShare(candidate));
      });
    ...
    return (async () => {
      ...
      return firestore.runTransaction(async (transaction) => {
        ...
        const updatedSessions = sessionDocs.filter((doc) => doc.exists).map((doc) => {
          const session = doc.data();
          return { ...session, members: synchronizeSessionMembersWithGroup(session, group.members) };
        });
        await assertFirestoreAccountWritesAllowed(firestore, transaction, recordAccountIds(group, ...updatedSessions));
  ...
  deleteGroupBill(groupId, billId, actorId) {
    if (isTesting) {
      ...
      const linkedSession = bill.sessionId ? data.sessions?.[bill.sessionId] : null;
      if (getBillStatus(bill) !== BILL_STATUS.ACTIVE || linkedSession?.members?.some((member) => member.active !== false && hasFinishedShare(member))) {
