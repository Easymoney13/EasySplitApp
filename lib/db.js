...
const db = {
  ...
  saveUser(user, trustedUid = '') {
    ...
    if (!uid) return null;
    const normalizedUser = enrichUserPhoneIdentity({ ...user, id: uid });
    if (isTesting) {
      const data = readDb();
  ...
  deleteUserAccountData(uid) {
    ...
    if (isTesting) {
      ...
      const phoneHmac = user?.phone ? identityHmac(`phone:${user.phone}`) : '';
    ...
    return (async () => {
      ...
      // Account deletion is deliberately exhaustive. It is rare and privacy-sensitive,
      // so a full scan is preferable to leaving a UID embedded in a legacy room shape.
      for (const collectionName of ['groups', 'sessions', 'history']) {
        const snapshot = await firestore.collection(collectionName).get();
        for (const doc of snapshot.docs) {
          const result = anonymizeAccountInRecord(doc.data(), uid, accountMemberReplacements);
          if (!result.changed) continue;
          anonymizedRecords += 1;
          await writer.enqueue((batch) => batch.set(doc.ref, sanitizeFirestoreData(result.record)));
        }
      }

      const deletedVisitIds = new Set();
      const visitSnapshots = [];
      visitSnapshots.push(await firestore.collection('restaurant_visits').where('userId', '==', uid).get());
      visitSnapshots.push(await firestore.collection('restaurant_visits').where('identityAliases', 'array-contains', `user:${uid}`).get());
      if (phoneHmac) {
        visitSnapshots.push(await firestore.collection('restaurant_visits').where('phoneHmac', '==', phoneHmac).get());
      }
      for (const snapshot of visitSnapshots) {
        for (const doc of snapshot.docs) {
          if (deletedVisitIds.has(doc.id)) continue;
          deletedVisitIds.add(doc.id);
          await writer.enqueue((batch) => batch.delete(doc.ref));
        }
      }
      ...
      for (const doc of observations.docs) {
        ...
        if (changed) await writer.enqueue((batch) => batch.set(doc.ref, sanitizeFirestoreData(observation)));
      }

      const historyPointers = await userRef.collection('history').get();
      for (const doc of historyPointers.docs) {
        await writer.enqueue((batch) => batch.delete(doc.ref));
      }
      if (userSnapshot.exists) await writer.enqueue((batch) => batch.delete(userRef));
      await writer.flush();
