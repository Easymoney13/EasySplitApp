...
async function getRegisteredRoom(firestore, roomType, code) {
  if (!/^\d{8}$/.test(String(code || ''))) return null;
...
const db = {
  ...
  generateUniqueRoomCode(roomType, roomId) {
    if (isTesting) {
      const data = readDb();
      return createUniqueRoomCode(data);
    }
    return (async () => {
  ...
  saveGroupBillAndSession(groupId, bill, session, actorId, expectedRevision = null) {
    if (isTesting) {
      ...
      if (currentBill?.status === 'settled') {
        const error = new Error('A settled bill cannot be edited');
        error.statusCode = 409;
        throw error;
      }
  ...
  deleteGroupBill(groupId, billId, actorId) {
    if (isTesting) {
      ...
      if (bill.status === 'settled' || linkedSession?.members?.some((member) => member.active !== false && member.settled === true)) {
        throw Object.assign(new Error('A paid or settled bill cannot be deleted'), { statusCode: 409 });
      }
