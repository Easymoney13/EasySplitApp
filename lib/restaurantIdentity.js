...
function createRestaurantIdentity(metadata = {}, fallbackName = '', fallbackKey = '', options = {}) {
  ...
  return {
    ...
    fieldVerification,
    fieldTrust,
    // A session-scoped identifier is useful for retaining evidence, but it is
    // not a venue identity that can safely be joined across receipts.
    venueResolutionStatus: ['business_id_address', 'name_phone', 'name_address'].includes(identityBasis)
      ? 'resolved'
      : 'unresolved',
