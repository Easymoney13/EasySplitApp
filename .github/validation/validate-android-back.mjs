...
async function connectWebView() {
  ...
  return await waitFor('EasySplit WebView page', async () => {
    ...
    return pages.find((page) => page.type === 'page' && page.url === 'https://localhost/')
...
async function main() {
  ...
  await sleep(1_500);
  if (!isEasySplitFocused()) {
    throw new Error('Back from the Start Split sheet backgrounded the app');
  }
  ...
  // Once the sheet closes, the Capacitor callback must be disabled again so
  // Android owns normal predictive-back behavior on the home screen.
  await sleep(1_000);
  adb('shell', 'input', 'keyevent', '4');
  await sleep(1_000);
  if (isEasySplitFocused()) {
    throw new Error('Back on the home screen remained intercepted after sheet cleanup');
  }
  ...
  console.log('ANDROID_BACK_HOME_SYSTEM_OWNED=PASS');
