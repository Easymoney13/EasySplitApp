...
export const AttachToGroupModal: React.FC<AttachToGroupModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn text-slate-900 dark:text-white">
      <div role="dialog" aria-modal="true" aria-label={t('attachBillTitle', undefined, 'Attach Bill to Group')} className="w-full max-w-sm photo-card p-6 bg-white dark:bg-brand-950 border border-slate-200 dark:border-slate-800 space-y-5 shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80 pb-3">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-2xl bg-brand-100 dark:bg-brand-950/80 text-brand-600 dark:text-brand-400">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <button
                    ...
                    className={`w-full p-3 rounded-xl border text-left flex items-center justify-between transition-all ${
                      selectedGroupId === g.id
                        ? 'border-brand-500 bg-brand-50/50 dark:bg-brand-950/40 text-slate-900 dark:text-white'
                        : 'border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300'
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full bg-brand-600 dark:bg-brand-300 text-white dark:text-brand-950 flex items-center justify-center font-black text-xs">
                    ...
                    {selectedGroupId === g.id && <Check className="w-4 h-4 text-brand-600 dark:text-brand-400 stroke-[3]" />}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="pt-2 flex items-center gap-3">
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-full bg-brand-600 dark:bg-brand-300 text-white dark:text-brand-950 font-black text-xs hover:bg-brand-700 dark:hover:bg-brand-200 flex items-center justify-center gap-1.5 shadow-md active:scale-95 disabled:opacity-40"
            >
